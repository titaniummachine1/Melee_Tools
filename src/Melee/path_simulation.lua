...
function PathSimulation.simulateDash(params)
	...
	local tickInterval = params.tickInterval or globals.TickInterval()
	...
	for tick = 1, params.ticks do
		...
		if traceHull then
			...
			local groundTrace = traceHull(nextPos, nextPos - (onGround and step or Vector3(0, 0, 0)), hull.MIN, hull.MAX, params.collisionMask, shouldHit)
...
function PathSimulation.simulateBonusTicks(params)
	...
	assert(params.startPos and params.startVel and params.enemyPos and params.enemyBack, "simulateBonusTicks: missing vectors")
